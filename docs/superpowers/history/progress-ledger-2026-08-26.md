...
--- Register truth + payee naming wave (plan: docs/superpowers/plans/2026-08-26-register-truth-and-payee-naming.md, branch register-truth, base 33c20a9) ---
Task 1: complete (3fd2796 + fix 07b0635). Review found CRITICAL: removing pendingBlocksReconcile made reconciled+unreviewed reachable and rejectTransaction had NO reconciled lock -> a locked row could be deleted out of a closed reconciliation. Fixed. Also 7 lying comments, a vacuous-on-regression suite (fixed with a cast test, verified red), and two invoice-linking guards that still treated unreviewed rows as not-money. Implementer caught a plan defect: my pinning test omitted enteredAt so it would have passed pre-fix.
Task 2: complete (e8e6aaa + fix d3a2058, review clean). Fixed: isPending->isUnreviewed (the identifier contradicted the restored 'Pending'=uncleared header), desktop payee font-medium overrode bold, phone had no dot, 2 stale comments.
Task 3: complete (1070200, review clean; design note: a SHORT known payee can substring-match an unrelated merchant, e.g. 'Ace' in 'Palace' — bounded by confirm-before-write).
Task 4: complete (7201c35 + fix 15a6b39). Review found: correcting a name twice created an alias keyed on the DISPLAY name that never fires -> rows land uncategorized with no chip to fix it (fixed: a correction updates the existing alias in place); the checkbox understated a bulk rename and hid the count (relabelled + renamedNotice); the bulk rename's reconciled carve-out was undocumented (now documented, and rejectTransaction's false 'every other mutator' claim corrected); the alias read failed open (owner-scoped, null = do not offer).
Task 5 walkthrough: PASS — inline by date; bold + rail dot, no chip; '1 to review · Enter All'; 'PENDING (1)' = the UNCLEARED row; DROPDOWN FIXED (opacity 1, zero stacking traps in ancestry, menu topmost at its own coords); suggestion chip 'Use "ZZ Starbucks"'; relabelled checkbox; alias written NORMALIZED; row renamed; Enter All cleared the marker and left 'uncleared' untouched. Sandbox cleaned.

SHIPPED 2026-08-26: register-truth + payee naming. Prod 0048 applied FIRST and verified, merged 33c20a9..482e5f7, pushed, smoke money=307 budget=307 poll 1. 930 tests.
  PARITY GATE — HALF PASSED, Dan chose to leave it live and fix the data:
  * RTA landed EXACTLY on the -146.84 prediction (arithmetic confirmed).
  * 3 categories differ, which the prediction said would stay exact. NOT arithmetic: Dan has Walmart(-41.32 Gig Expenses), UA(-8.00 Flights) and Starbucks(-25.00 Meals) CATEGORIZED IN YNAB while all 6 imported rows are uncategorized in the app. My prediction wrongly assumed YNAB had them uncategorized too.
  * Convergence path: filing those 3 in the app zeroes the category deltas and moves RTA to -72.52. The remaining RTA gap vs YNAB (592.10) is the Fairmont refund, treated differently in the two books.
