# Session progress ledger — archived snapshot, 2026-08-24

A committed copy of `.superpowers/sdd/progress.md` (which is gitignored
scratch and otherwise lives only on the machine that wrote it), so a session
on ANY machine — including Claude Code cloud — inherits the full build
history: every wave, every review verdict, every accepted trade. Append-only
history; the live ledger keeps being the working copy locally.
